...
_OCR_IMAGE_EXTENSIONS = {".jpg", ".jpeg", ".png", ".bmp", ".gif", ".webp"}


def resolve_ocr_root(base_dir: Path, input_dir: str) -> Path:
    """JSON input_dir 을 OCR 루트 Path 로 해석합니다."""
    p = Path(input_dir)
    return p if p.is_absolute() else base_dir / p


def _ordered_ocr_page_dirs(ocr_root: Path) -> list[Path]:
    """
    inference_qwen3vl32b_lora.load_ocr_data_from_pages 와 동일한 순서의 페이지 디렉터리 목록.
    flat: input_dir/0001, nested: input_dir/문서명/0001
    """
    ...
    subdirs = [
        ...
        if (ocr_root / name).is_dir()
    ]
    numeric = [(n, path) for n, path in subdirs if n.isdigit()]
    non_numeric = [(n, path) for n, path in subdirs if not n.isdigit()]

    entries: list[tuple[int, Path, str]] = []
    if numeric and not non_numeric:
        for name, path in numeric:
            entries.append((int(name), path, name))
        entries.sort(key=lambda x: x[0])
    else:
        for doc_name, doc_path in sorted(non_numeric):
            for page_item in sorted(os.listdir(doc_path)):
                page_path = doc_path / page_item
                if page_path.is_dir() and page_item.isdigit():
                    entries.append((int(page_item), page_path, f"{doc_name}/{page_item}"))
        entries.sort(key=lambda x: (x[2].rsplit("/", 1)[0], x[0]))

    return [path for _, path, _ in entries]


def _ocr_page_dir_has_content(page_dir: Path) -> bool:
    """텍스트(result.mmd) 또는 images/ 내 이미지가 하나라도 있으면 True (추론 스크립트와 동일)."""
    mmd = page_dir / "result.mmd"
    if mmd.exists():
        try:
            if mmd.read_text(encoding="utf-8").strip():
                return True
        except OSError:
            pass
    ...
    if images_dir.is_dir():
        for f in os.listdir(images_dir):
            if Path(f).suffix.lower() in _OCR_IMAGE_EXTENSIONS:
                return True
    return False


def list_included_ocr_page_dirs(ocr_root: Path) -> list[Path]:
    """
    추론 시 pages_data 에 들어가는 순서와 동일한 페이지 디렉터리만 모읍니다
    (빈 OCR·빈 이미지 폴더는 스킵).
    """
    return [p for p in _ordered_ocr_page_dirs(ocr_root) if _ocr_page_dir_has_content(p)]
...
def main():
    ...
    ocr_root = resolve_ocr_root(base_dir, input_dir) if input_dir else None
    included_page_dirs = (
