"""
conda Qwen3-VL-train 환경
Qwen3-VL 원본 모델 추론 예제 (LoRA 미사용)
...
def load_model():
    """
    원본 모델과 토크나이저 로드 (LoRA 어댑터 없이)

    Returns:
        model, tokenizer: 로드된 모델과 토크나이저
    """
    print("📥 원본 모델 로드 중 (LoRA 미사용)...")

    # Base model 로드 (LoRA 없이)
    model, tokenizer = FastVisionModel.from_pretrained(
        "unsloth/Qwen3-VL-8B-Instruct-unsloth-bnb-4bit",
        load_in_4bit=True,
        use_gradient_checkpointing=False,  # 추론 시에는 불필요
    )

    # LoRA 어댑터를 추가하지 않고 바로 추론 모드로 설정
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen3-VL 원본 모델로 QA 생성 (LoRA 미사용)"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    print("=" * 80)
    print("Qwen3-VL 원본 모델로 QA 생성 (LoRA 미사용)")
    print("=" * 80)
